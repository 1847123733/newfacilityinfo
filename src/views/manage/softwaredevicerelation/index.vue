...
<script>
import {  getDevicesBySoftwareId,softwaredevicerelation,getUpdateDevicesBySoftwareId, delSoftwaredevicerelation, addSoftwaredevicerelation,  exportSoftwaredevicerelation } from "@/api/manage/softwaredevicerelation";
...
export default {
  ...
  methods: {
    ...
    submitForm1() {
       ...
       let ids2 =  this.ids1.join()
      // console.log(this.queryParams.versionId)
      // console.log(typeof this.queryParams.versionId)
      this.loading = true;
      softwaredevicerelation({
        deviceIds:ids2,
        softwareId:this.queryParams.versionId
      }).then(response => {
        // this.listProduct = response.rows;
        this.msgSuccess("添加成功");
        console.log(response,'更新升级对应的设备')
        this.getDevicesBySoftware()
        this.open = false;
        this.getList()
        this.loading = false;
